...
- 修正: 2文字未満の値は `protect_text()` の自動置換対象外にする
...
| 項目 | 詳細 | 参照 |
|------|------|------|
| memory 重複記憶抑制 | 同一factの重複保存防止 | §10.2 #3 |
| httpx.AsyncClient 再利用 | KeepAlive/TCP再利用が効かない問題 | §10.2 #4 |
...
| # | 内容 | 条件 |
|---|------|------|
| 1 | ~~`History.save_turn()` が `_messages[-2:]` を前提~~ → ✅ v3.10 解決済み | — |
...
| 5 | ~~設定APIバリデーション~~ → ✅ v3.10 解決済み | — |
| 6 | ~~属性エスケープ~~ → ✅ v3.10 解決済み | — |
